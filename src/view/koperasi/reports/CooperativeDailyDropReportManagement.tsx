...
import { useCooperativeDailyDropReport } from '@/hooks/useCooperativeDailyDropReport';
import { useI18n } from '@/hooks/useI18n';
import dayjs from '@/lib/dayjs';
import { exportCsv, exportHtmlPdf, saveExportFile, type ExportRows, type ExportTarget } from '@/utils/export';
import CooperativeDailyDropReport from './CooperativeDailyDropReport';
...
const getOfficerLabel = (
  ...
  officerPosition: string | undefined,
  unassignedLabel: string,
) => {
  if (!officerName) return unassignedLabel;

  return officerPosition ? `${officerName} - ${officerPosition}` : officerName;
};

export default function CooperativeDailyDropReportManagement() {
  ...
  const reportQuery = useCooperativeDailyDropReport(filters);
  ...
  const labels = {
    title: t('cooperative.reports.dailyDrop.title'),
  ...
  const buildCsvRows = (): ExportRows => {
    ...
    report.groups.forEach((group) => {
      ...
      rows.push([
        ...
        getOfficerLabel(group.officer_name, group.officer_position, t('cooperative.memberRegister.unassignedEmployee')),
      ]);
      rows.push([
        labels.date,
        labels.day,
  ...
  const handleExportCsv = async (target: ExportTarget = 'auto') => {
    ...
    try {
      const exported = await exportCsv({
        filename: `laporan-drop-harian-${dayjs().tz().format('YYYY-MM-DD')}.csv`,
        rows: buildCsvRows(),
        target,
      });
      if (!exported) return;
      message.success(t('cooperative.reports.dailyDrop.exportCsvSuccess'));
    } catch (error) {
      console.error('Failed to export daily drop CSV:', error);
      message.error(t('cooperative.reports.dailyDrop.exportCsvFailed'));
    }
  };
  ...
  const handleExportHtml = async (target: ExportTarget = 'auto') => {
    ...
    try {
      const exported = await saveExportFile({
        filename: `laporan-drop-harian-${dayjs().tz().format('YYYY-MM-DD')}.html`,
        mimeType: 'text/html',
        content: buildHtmlDocument(),
        target,
      });
      if (!exported) return;
      message.success(t('cooperative.reports.dailyDrop.exportHtmlSuccess'));
    } catch (error) {
      console.error('Failed to export daily drop HTML:', error);
      message.error(t('cooperative.reports.dailyDrop.exportHtmlFailed'));
    }
  };
  ...
  const handleExportPdf = async (target: ExportTarget = 'auto') => {
    ...
    try {
      const exported = await exportHtmlPdf({
        filename: `laporan-drop-harian-${dayjs().tz().format('YYYY-MM-DD')}.pdf`,
        element: reportRef.current,
        orientation: 'landscape',
        target,
      });
      if (!exported) return;
      message.success(t('cooperative.reports.dailyDrop.exportPdfSuccess'));
    } catch (error) {
      console.error('Failed to export daily drop PDF:', error);
      message.error(t('cooperative.reports.dailyDrop.exportPdfFailed'));
    }
  };
    ...
    <div className="space-y-4 p-3 sm:p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <Text type="secondary">{t('cooperative.reports.dailyDrop.subtitle')}</Text>
        </div>
        <Space wrap>
