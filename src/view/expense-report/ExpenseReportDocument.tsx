...
const ExpenseReportDocument = forwardRef<HTMLDivElement, ExpenseReportDocumentProps>(
  function ExpenseReportDocument({
    transactions,
    totalExpense,
    breakdown,
    companyName,
    ...
    return (
      <div ref={ref} style={reportWrapperStyle} data-testid="expense-report-document">
          ...
          <div style={reportIdentityStyle}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: '#4b5563', fontSize: 13, marginTop: 2 }}>
                {t('report.expense.subtitle')}
              </div>
            </div>
          </div>
          <div style={reportMetaStyle}>
            <div>{t('report.periodWithColon')} {periodText}</div>
            <div>{t('report.expenseCategory')}: {categoryText}</div>
        ...
        <div style={summaryGridStyle}>
          ...
          <div style={summaryCellStyle}>
            <div style={summaryLabelStyle}>{t('report.expenseCategory')}</div>
            <div style={summaryValueStyle}>{breakdownRows.length}</div>
          </div>
          <div style={{ ...summaryCellStyle, borderRight: 0 }}>
            <div style={summaryLabelStyle}>{t('report.totalExpense')}</div>
            <div style={summaryValueStyle}>{money(totalExpense)}</div>
          </div>
        </div>

        <section style={{ breakInside: 'avoid', marginBottom: 18 }}>
          <h3 style={sectionTitleStyle}>{t('report.expense.categorySummary')}</h3>
          <table style={tableStyle}>
            ...
            <tbody>
              ...
              <tr style={{ background: '#f9fafb', fontWeight: 700 }}>
                <td style={cellStyle}>{t('report.totalExpense')}</td>
                <td style={numberCellStyle}>{money(totalExpense)}</td>
              </tr>
            </tbody>
          </table>
        </section>
        ...
        <section>
          <h3 style={sectionTitleStyle}>{t('report.expense.transactionDetails')}</h3>
          <table style={tableStyle}>
            <thead>
              <tr>
...
export default ExpenseReportDocument;
