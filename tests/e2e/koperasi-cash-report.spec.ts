...
test('laporan tunai harian per karyawan dan export', async ({ page }) => {
  ...
  await expect(page.getByTestId('koperasi-cash-report-date-filter').locator('input')).not.toHaveValue('');
  ...
  await expect(rows.nth(1).locator('td').nth(0)).toHaveText('DROPING');
  await expect(rows.nth(1).locator('td').nth(1)).toContainText('Rp 1.000.000');
  await expect(rows.nth(1).locator('td').nth(2)).toContainText('Rp 600.000');
  ...
  await expect(rows.nth(3).locator('td').nth(0)).toHaveText('JUMLAH');
  await expect(rows.nth(3).locator('td').nth(1)).toContainText('Rp 1.600.000');
  await expect(rows.nth(3).locator('td').nth(2)).toContainText('Rp 650.000');
  await expect(employeeReport).toContainText('TOTAL SALDO');
  await expect(employeeReport).toContainText('Rp 950.000');
