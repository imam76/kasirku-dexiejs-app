...
pub async fn postgres_list_hr_positions(
    ...
    Ok(hr_repository::list_hr_positions(&state.pool()?).await?)
}
...
pub async fn postgres_list_employment_contracts(
    ...
    Ok(hr_repository::list_employment_contracts(&state.pool()?).await?)
}
...
pub async fn postgres_list_salary_components(
    ...
    Ok(hr_repository::list_salary_components(&state.pool()?).await?)
}
...
pub async fn postgres_list_employee_salary_components(
    ...
    Ok(hr_repository::list_employee_salary_components(&state.pool()?).await?)
}
