...
- Workflow sales document yang sudah mengirim bundle header/item: create draft, update draft, issue, convert, dan void. Side effect stok tetap lewat `stock_mutations`; journal dari issue/void sales invoice sekarang ikut lewat `journal_entries`.
...
- Workflow purchase document yang sudah mengirim bundle header/item: create draft, update draft, issue, convert, dan void. Side effect stok tetap lewat `stock_mutations`; payment ledger AP tetap belum dipindahkan ke PostgreSQL, sedangkan journal dari AP payment masuk lewat `journal_entries`.
...
- Journal/general ledger sudah masuk sebagai Fase 17 poin 11: header `journal_entries` dan detail `journal_entry_lines` disinkronkan sebagai satu bundle lewat queue, read refresh, `version`, actor metadata, dan optimistic concurrency berdasarkan `version` + `updated_at`.
- Workflow yang sudah mengirim journal bundle: opening balance, manual journal, POS sale/void, stock purchase, Sales Invoice issue/void, AR/AP invoice payment dan void payment, Sales Return issue/void, serta Cash & Bank transfer/void transfer.
- `financeBalance` tetap agregat lokal Dexie yang direkalkulasi dari `financeTransactions` saat read refresh; `generalLedgerSetting`, COA, dan report agregasi server-side belum dipindahkan sebagai bagian poin 11. Report server-side tetap masuk poin 12.
